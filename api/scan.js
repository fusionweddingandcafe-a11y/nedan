...
{"price":<整数|null>,"tax_status":"included"|"excluded"|"unknown","tax_rate":10,"confidence":"high"|"medium"|"low","reasoning":"<30字以内>","ignored":[{"value":<数値>,"reason":"<理由>"}]}`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: `Anthropic API error: ${response.status}`, detail: errText.slice(0, 200) });
    }
    ...
    try { result = JSON.parse(clean); }
      ...
      else return res.status(500).json({ error: 'JSON parse failed', raw: clean.slice(0, 200) });
    }
