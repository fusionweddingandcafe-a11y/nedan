...
export default async function handler(req, res) {
  ...
  if (!req.body) return res.status(400).json({ error: 'body is null' });
  const { image } = req.body;
  if (!image) return res.status(400).json({ error: 'image missing', keys: Object.keys(req.body||{}) });
...
{"price":<整数|null>,"tax_status":"included"|"excluded"|"unknown","tax_rate":10,"confidence":"high"|"medium"|"low","reasoning":"<30字>","ignored":[]}`;
  ...
  try {
    ...
    catch { const m=clean.match(/\{[\s\S]*\}/); if(m) return res.status(200).json(JSON.parse(m[0])); return res.status(500).json({error:'parse failed',raw:clean.slice(0,200)}); }
  } catch(e) {
